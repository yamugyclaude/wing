...
**정리**: 150%와 100%는 WIDTH 값, 40과 -100은 PAN 값입니다. 두 컨트롤은 채널 HOME 화면의 MAIN SENDS 탭 중앙 원형 그래픽(WIDTH/PAN)에서 함께 조정합니다.`,
  ...
  {
    slug: "sof-flip-workflow",
    question: "SOF FLIP은 정확히 어떻게 쓰는 건가요? (에디터엔 물리 버튼이 없어서 헷갈림)",
    summary: "SOF FLIP은 채널을 선택하는 기능이 아니라, 페이더를 잠깐 '센드 레벨 컨트롤'로 바꿔주는 기능",
    confirmed: true,
    source: "User Manual Section 4.9 Sends On Faders",
    answer: `**오해하기 쉬운 부분**: SOF FLIP은 "묶을 채널을 선택하는" 기능이 아니라, **왼쪽 채널 페이더 전체를 그 순간만 다른 용도(센드 레벨 컨트롤)로 바꿔주는** 기능입니다.

**정확한 순서:**
1. 중앙(Bus) 또는 오른쪽(Main/Matrix) 페이더 뱅크에서 원하는 Bus를 SELECT
2. SOF FLIP 버튼을 누름
3. 이제 왼쪽 12개 채널 페이더가 "각 채널이 그 Bus로 보내는 센드 레벨" 컨트롤로 바뀜
4. 페이더를 올리면 센드 레벨이 올라가고, 채널의 MUTE 버튼을 누르면 그 채널의 센드가 꺼짐/켜짐 (선택이 아니라 뮤트가 on/off 스위치 역할)
5. 다 끝나면 SOF FLIP을 다시 눌러 종료 — 안 끄면 페이더가 계속 센드 컨트롤 상태로 남아 평소 볼륨 조작과 헷갈림

**WING Edit(에디터)에 물리 SOF FLIP 버튼이 없는 이유**: 이 기능은 "물리 페이더를 잠깐 다른 용도로 빌려 쓰는" 하드웨어 전용 단축 기능이라, 마우스로 조작하는 에디터에선 애초에 필요가 없습니다. 에디터에서는 Bus/Main/Matrix의 **Feed Configuration 화면**에서 채널을 선택하고 SEND LEVEL을 직접 입력하면 SOF와 같은 결과를 얻을 수 있습니다.

**예: Bus 1에 보컬들을 묶는 실전 행동 패턴**

① 사전 설정 — Bus 1을 보컬 전용으로 지정 (공연 전 한 번만)
1. 중앙 페이더 뱅크에서 Bus 1 SELECT
2. Bus 1의 HOME 화면 → 중앙 그리드 클릭 → Bus 1 Feed Configuration 진입
3. 그리드에서 보컬 채널들 전부 선택
4. SET SEND MODE → TAP/PRE
5. SEND ENABLE → SET ON
6. (필요하면) IGNORE CHANNEL MUTE → SET ON — PA에서 보컬 뮤트해도 모니터는 유지
7. APPLY

② 실전 레벨 조정 — 공연 중 밸런스 잡을 때 (반복적으로 사용)
1. Bus 1 SELECT (①에서 이미 선택돼 있으면 생략)
2. SOF FLIP 버튼 누름
3. 왼쪽 채널 페이더가 Bus 1 센드 레벨 컨트롤로 바뀜 — 보컬이 다른 뱅크에 있으면 그 뱅크로 전환
4. 보컬 페이더들을 움직여서 모니터 밸런스 조정
5. 끝나면 SOF FLIP 다시 눌러서 종료

**핵심 구분**: ①은 "연결을 만드는 것"(한 번만), ②는 "그 연결의 볼륨을 실시간으로 만지는 것"(반복). SOF FLIP은 ②에서만 의미가 있습니다 — ①이 안 된 상태에서 SOF FLIP을 눌러봐야 아직 연결이 없어서 아무 반응이 없습니다.`,
  },
];
