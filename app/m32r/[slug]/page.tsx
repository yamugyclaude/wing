...
export default async function M32rQnaPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const item = m32rQnaList.find((q) => q.slug === slug);
  if (!item) notFound();

  return (
    <div className="max-w-2xl mx-auto w-full px-4 py-10">
      <Link href="/m32r" className="text-sm text-neutral-500 underline underline-offset-4">
        ← M32R 목록으로
      </Link>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        {item.source && <span className="text-xs text-neutral-500">{item.source}</span>}
      </div>
